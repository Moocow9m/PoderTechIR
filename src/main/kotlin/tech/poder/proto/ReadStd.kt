...
object ReadStd {
    ...
    fun readAny(stream: InputStream): Any {
        return when (val code = Packet.Types.values()[max(min(readVUInt(stream).toInt(), Packet.Types.UNKNOWN.ordinal), 0)]) {
            Packet.Types.VUINT -> readVUInt(stream)
            Packet.Types.VINT -> readVInt(stream)
            Packet.Types.STRING -> readString(stream)
            Packet.Types.PACKET -> {
                val packet = ReadStd.javaClass.classLoader.loadClass(readString(stream)) ?: throw Exception("Unknown packet class")
